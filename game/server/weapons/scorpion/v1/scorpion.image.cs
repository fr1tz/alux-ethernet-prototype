...
datablock ParticleData(WpnScorpionFireEmitter_Particles)
{
	dragCoefficient       = 1;
...
datablock ParticleEmitterData(WpnScorpionFireEmitter)
{
	ejectionPeriodMS = 10;
	periodVarianceMS = 0;
	...
	lifetimeMS       = 0;
	particles        = "WpnScorpionFireEmitter_Particles";
};
...
datablock ShapeBaseImageData(WpnScorpionImage)
{
	// add the WeaponImage namespace as a parent
	className = WeaponImage;

	// basic item properties
	shapeFile = "share/shapes/alux/scorpion.image.dts";
	emap = true;
	...
	usesEnergy = true;
	minEnergy = WpnScorpionPseudoProjectile.energyDrain-1;

	projectile = WpnScorpionPseudoProjectile;
	...
	crosshair = "blaster"; // crosshair to display when holding this weapon
 	fireprojectile[0] = WpnScorpionProjectile;
   reloadTime = 2000;
	...
	//
		...
		stateFire[3]                     = true;
		stateFireProjectile[3]           = WpnScorpionPseudoProjectile;
...
function WpnScorpionImage::getBulletSpread(%this, %obj)
{
   return 0.0;
}

function WpnScorpionImage::onMount(%this, %obj, %slot)
{
   ...
   %obj.setImageCurrentRecoil(%slot, 15);
   %obj.setImageMaxRecoil(%slot, 15);
